...
class PlayViewModel(
    ...
    private val isFinish: (Target, ObjectState) -> Boolean,
    ...
    fun start(satellite: ObjectState) {
        satelliteLocationJob = launch {
            ...
            for (time in timer) {
                ...
                if (isFinish(target, state)) {
                    satellitePosition.postValue(ViewState.failure(R.string.info_finish))
                    break
                } else if (hasCollision(planet, state)) {
